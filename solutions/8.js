...
let input = readline.createInterface({
  input: fs.createReadStream('input/8.txt')
});
...
input.on('line', line => {
	...
	for (let y = 0; y < grid.length; y++) {
		if (y == 0 || y == grid.length - 1) { // edge
			sum += grid[y].length;
			continue;
		}

		for (let x = 0; x < grid[y].length; x++) {
			if (x == 0 || x == grid[y].length - 1) { // edge
				sum++;
				continue;
			}
			...
			function checkX(x, d) {
				const row = grid[y];
				const t = row[x];
				for (x += d; x < row.length && x >= 0; x += d) {
					if (row[x] >= t) return false;
				}
				return true;
			}
			...
			function calcView () {
				function viewX (x, d) {
					const row = grid[y];
					const o = x;
					for (x += d; x < row.length - 1 && x > 0; x += d) {
						if (grid[y][x] >= grid[y][o]) {
							break;
						}
					}
					return Math.abs(o - x);
				...
				function viewY(y, d) {
					const o = y;
					for (y += d; y < grid.length - 1 && y > 0; y += d) {
						if (grid[y][x] >= grid[o][x]) {
							break;
						}
					}
					return Math.abs(o - y);
				...
				return viewX(x, 1) * viewX(x, -1) * viewY(y, 1) * viewY(y, -1);
			}

			if (checkX(x, 1) || checkX(x, -1) ||
				checkY(y, 1) || checkY(y, -1)) {
				sum++;
