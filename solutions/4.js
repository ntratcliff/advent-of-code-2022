...
function part1() {
	...
	let input = readline.createInterface({
		input: fs.createReadStream('input/4.txt')
	});

	input.on('line', (line) => {
		var pair = line.split(',').map(v => v.split('-').map(v => parseInt(v)));

		var outer = Math.sign(pair[0][0] - pair[1][1]);
		var inner = Math.sign(pair[0][1] - pair[1][0]);
		var lhs = Math.sign(pair[0][0] - pair[1][0]);
		var rhs = Math.sign(pair[0][1] - pair[1][1]);

		if (outer + inner + lhs + rhs === 0 || outer !== inner && lhs !== rhs) {
			count++;
		}

	}).on('close', () => console.log(`P1: ${count}`));
}
...
function part2() {
	...
	input.on('line', (line) => {
		...
		if (Math.abs(outer + inner + lhs + rhs) < 4) {
			count++;
		}

	}).on('close', () => console.log(`P2: ${count}`));
}
