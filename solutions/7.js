...
let input = readline.createInterface({
  input: fs.createReadStream('input/7.txt')
});
...
input.on('line', line => {
  ...
  if (line.startsWith('dir') || line.startsWith('$ ls')) return;

  current[0] += parseInt(line.split(' ')[0]);

}).on('close', () => {
  // finish processing stack
  while (stack.length > 0) leaveDir();

  // flatten and reduce tree to sum of directories w/ size < 100000 (part 1)
  const flat = tree.flat(Infinity);
  console.log("P1: " + flat.reduce((a, v) => a += v <= 100000 ? v : 0, 0));

  // get smallest directory that's greater than the required amount of space (part 2)
  console.log("P2: " + flat.reduce((m, v) => m = v > 30000000 - (70000000 - tree[0]) && v < m ? v : m, Infinity)); 
});