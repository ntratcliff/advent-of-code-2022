...
function part1() {
	...
	let input = readline.createInterface({
		input: fs.createReadStream('input/3.txt')
	});

	input.on('line', (line) => {
		// find the first matching character from the set of characters in the back half of the line
		var match = line.match(`[${line.slice(line.length / 2, line.length)}]`);

		// add character value to sum
		sum += getPriority(line.charCodeAt(match.index));
	}).on('close', () => console.log(`P1: ${sum}`));
}
...
function part2() { 
	...
	let count = 0;
	input.on('line', (line) => {
		// seed first line for matching
		if (count++ == 0) {
			current = line;
			return;
		}

		// get all characters matching the set of characters in current as a string
		current = [...line.match(new RegExp(`[${current}]`, 'g'))].join('');

		// sum and reset for next set of 3
		if (count == 3) {
			sum += getPriority(current.charCodeAt(0));
			count = 0;
		}
	}).on('close', () => console.log(`P2: ${sum}`));
}
