...
function getStart(stream, buffSize) {
	const len = buffSize - 1;
	var buffer = '';
	for (var i = 0; i < stream.length; i++) {
		const c = stream[i];
		const bi = buffer.indexOf(c);

		if (bi == -1 && buffer.length == len) {
			return i + 1;
		}
		
		if (bi > -1) {
			buffer = buffer.substring(bi + 1);
		}

		buffer = buffer.concat(c);
		buffer = buffer.substring(buffer.length - len);
	}

	return -1;
}

const stream = fs.readFileSync('input/6.txt').toString();
console.log(`P1: ${getStart(stream, 4)}`);
...
console.log(`P2: ${getStart(stream, 14)}`);