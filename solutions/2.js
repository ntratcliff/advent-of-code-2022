...
function getScore(match) {
	/*
		compare the value of index 0 (opponent) with us (1)
		N v N = draw
		2 > 1
		3 > 2
		1 > 3
	*/
	...
	var matchScore = 0;
	if (match[0] === match[1]) { // draw
		matchScore = draw;
	}
	else if (match[0] === 1 && match[1] === 2 ||
			match[0] === 2 && match[1] === 3 || 
			match[0] === 3 && match[1] === 1) { // win!
		matchScore = win;
	}

	return match[1] + matchScore;
}
...
function part1() {
	...
	// read a line
	input.on('line', (line) => {
		// split the line by ' ' (space) and convert with value table
		var match = line.split(' ')
						.map(v => values[v]);
		score += getScore(match);
	}).on('close', () => console.log(`P1: ${score}`));
}
...
function part2() {
	function getMove(match) {
		...
		*/
		...
		throw new Error(`Invalid match. Values: ${match.join(", ")}`)
	}
	...
	input.on('line', (line) => {
		...
		match[1] = getMove(match);
		score += getScore(match);
	}).on('close', () => console.log(`P2: ${score}`));
}
