...
function part1() {
	...
	// loop through list and sum items until we get to an empty line
	input.on('line', (line) => {
		if (line.trim().length === 0) {
			// compare against stored max sum
			// replace max sum if current sum greater
			if (max < sum) {
				max = sum;
			}
			sum = 0;
...
function part2() {
	...
	input.on('line', (line) => {
		if (line.trim().length === 0) {
			// compare sum against each max value in array
			let inserted = false;
			for (let i = 0; i < max.length; i++) {
				if (max[i] < sum) {
					// insert at index if higher than current value
					max.splice(i, 0, sum);
					inserted = true;

					// keep max array length 3
					if (max.length > 3) {
						max.splice(3, 1);
					}
			...
			// push value if max length < 3
			if (!inserted && max.length < 3) {
				max.push(sum);
			}
	...
	}).on('close', () => console.log(`P2: ${max.reduce((a, v) => a + v, 0)}`));
}
